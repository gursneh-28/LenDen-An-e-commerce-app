...
import { requestAPI, getUser, requestRoomId } from "../../services/api";
...
function RequestCard({ item, onChat }) {
  return (
    <View style={s.card}>
...
export default function Help() {
  ...
  const applyFilters = (list, q, cat, sortType) => {
    ...
    if (q.trim()) {
      ...
      out = out.filter((r) => r.work?.toLowerCase().includes(lower) || r.requesterName?.toLowerCase().includes(lower));
    }
    ...
    <View style={s.screen}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <RequestCard item={item} onChat={() => handleChat(item)} />}
          contentContainerStyle={s.list}
          showsVerticalScrollIndicator={false}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchRequests(); }} tintColor="#1a1a1a" />}
        />
      )}
    </View>
  );
